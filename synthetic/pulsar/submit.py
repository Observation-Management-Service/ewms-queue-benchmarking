...
def create_jobs(queue_address, queue_auth, queue_name, server=None, access_token=None, pubs=1, workers=1, parallel=True, msgs_per_pub=1000, msg_size=100, delay=0, scratch=Path('/tmp'), venv=None, **kwargs):
    ...
    pub_jobs = schedd.submit(htcondor.Submit({
        ...
        'request_cpus': '1',
        'request_memory': '1GB',
        'arguments': f'python pub.py --auth {queue_auth} --server-address {server} {access} --num-msgs {msgs_per_pub} --msg-size {msg_size} --parallel {10 if parallel else 1} {queue_address} {queue_name}',
    }), count=pub_job_count)
    ...
    worker_jobs = schedd.submit(htcondor.Submit({
        ...
        'request_memory': '1GB',
        'priority': '1',
        'arguments': f'python worker.py --auth {queue_auth} --server-address {server} {access} --delay {delay} --parallel {10 if parallel else 1} {queue_address} {queue_name}',
...
def main():
    ...
    parser.add_argument('--queue-auth', default=None, help='auth token')
    ...
    delay = args['delay']
    queue_name = f'pulsar-p{pubs}-w{workers}-m{msg_size}-d{delay}'
